...
//        }
        ...
        nameLabel.snp.makeConstraints {
            $0.top.equalTo(view.safeAreaLayoutGuide).inset(Constants.verticalMargin)
            $0.centerX.equalTo(view.safeAreaLayoutGuide)
        }
    ...
    private func updateUI() {
        ...
        if let yellowOilPrice = oilPrices.first(where: { $0.prodcd == "B027" }) {
            yellowOilPriceLabel.text = "휘발유: \(String(yellowOilPrice.price).addedComma())원"
        } else {
            yellowOilPriceLabel.text = "휘발유 정보 없음"
        }

        if let greenOilPrice = oilPrices.first(where: { $0.prodcd == "D047" }) {
            greenOilPriceLabel.text = "경유: \(String(greenOilPrice.price).addedComma())원"
        } else {
...
//class GasStationDetailViewController: UIPresentationController {
//    private var gasStation: CustomGasStation?
//
//    private let nameLabel = {
//        let label = UILabel()
//        label.textColor = .black
...
//
//    init(presentedViewController: UIViewController, presenting presentingViewController: UIViewController?, size: CGFloat, data: CustomGasStation?) {
//        self.size = size
...
//        self.containerView?.addSubview(telLabel)
//        self.containerView?.addSubview(dateLabel)
//        self.containerView?.addSubview(storeImage)
//        self.containerView?.addSubview(carWashImage)
//        self.containerView?.addSubview(greenOilImage)
//        self.containerView?.addSubview(yellowOilImage)
//        self.containerView?.addSubview(greenOilPriceLabel)
//        self.containerView?.addSubview(yellowOilPriceLabel)
//
