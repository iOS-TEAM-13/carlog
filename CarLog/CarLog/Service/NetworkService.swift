                ...
                AF.request(urlString).responseDecodable(of: gasStationResponse.self) { response in
                    ...
                    case .success(let data):
                        if let data = data.result.oil.first {
                            result.append(CustomGasStation(id: data.uniID, address: data.newAdr, carWashYn: data.carWashYn, cvsYn: data.cvsYn, gisXCoor: data.gisXCoor, gisYCoor: data.gisYCoor, oilPrice: data.oilPrice))
                        }
                    case .failure(let error):
                        print(error)
                    }
